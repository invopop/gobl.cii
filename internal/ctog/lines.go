...
func ParseCtoGLines(transaction *structs.SupplyChainTradeTransaction) []*bill.Line {
	...
	for _, item := range items {
		price, _ := num.AmountFromString(item.SpecifiedLineTradeAgreement.NetPriceProductTradePrice.ChargeAmount)
		line := &bill.Line{
		...
		if item.SpecifiedLineTradeSettlement.SpecifiedTradeAllowanceCharge != nil {
			line = parseLineCharges(item.SpecifiedLineTradeSettlement.SpecifiedTradeAllowanceCharge, line)
		}
...
func parseLineCharges(allowances []*structs.SpecifiedTradeAllowanceCharge, line *bill.Line) *bill.Line {
